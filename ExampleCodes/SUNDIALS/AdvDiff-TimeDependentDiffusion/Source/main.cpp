...
void main_main ()
{
    ...
    for (int step = 1; step <= nsteps; ++step)
    {
        ...
        // Advance to output time. If this application interval lands on a
        // scheduled diffusion jump, reset SUNDIALS history at the jump before
        // continuing in the next coefficient segment.
        Real evolve_time = time_start;
    ...
    if (rl_data_enabled) {
        episode_valid = rl::RecordRunEnd(rl_output.get(), phi, completed_steps, nsteps,
                                         episode_valid, evolution_stop_time, analytic_error,
                                         mlmg_telemetry);
    }
    if (rl_stdin_json_control && amrex::ParallelDescriptor::IOProcessor()) {
